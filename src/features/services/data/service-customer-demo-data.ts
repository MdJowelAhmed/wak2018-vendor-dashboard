...
export function buildServiceCustomerList(
  ...
  const rows: CustomerListRow[] = [...map.values()].map((c) => {
    const tags: CustomerTag[] =
      c.totalOrders > 1 ? (["repeat_buyer"] as const) : [];
    return {
      id: c.id,
      name: c.name,
      avatarUrl: undefined,
...
export function buildServiceCustomerDetails(
  bookings: ServiceBooking[],
  customerId: string,
  ...
  const tags: CustomerTag[] =
    totalOrders > 1 ? (["repeat_buyer"] as const) : [];
